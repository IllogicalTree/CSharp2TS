...
export default {
  ...
  async get2(id: number): Promise<TestClass> {
    const response = await apiClient.get<TestClass>(`api/Test/${id}`);
    return response.data; 
  },

  async get3(id: number, externalId: number): Promise<TestClass> {
    const response = await apiClient.get<TestClass>(`api/Test/${id}?externalId=${externalId}`);
    return response.data; 
  },

  async generic(): Promise<GenericClass1<TestClass2>> {
    const response = await apiClient.get<GenericClass1<TestClass2>>(`api/Test`);
    return response.data; 
  },

  async getFiltered(filter: string, limit: number): Promise<TestClass[]> {
    const response = await apiClient.get<TestClass[]>(`api/Test/filtered?filter=${filter}&limit=${limit}`);
    return response.data; 
  },

  async create(testClass: TestClass): Promise<TestClass> {
    const response = await apiClient.post<TestClass>(`api/Test`, testClass);
    return response.data; 
  },

  async createFromBody(model: string): Promise<string> {
    const response = await apiClient.post<string>(`api/Test`, model);
    return response.data; 
  },

  async update(id: number, testClass: TestClass): Promise<TestClass> {
    const response = await apiClient.put<TestClass>(`api/Test/${id}`, testClass);
    return response.data; 
  },

  async partialUpdate(id: number, model: TestClass): Promise<TestClass> {
    const response = await apiClient.patch<TestClass>(`api/Test/${id}`, model);
    return response.data; 
  },
